      ...
      <CardContent class="space-y-6">
        ...
        <div class="space-y-4">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div class="flex items-center space-x-2 h-10 px-3 border rounded-md bg-card">
              <Checkbox id="audio_only" v-model:checked="form.audio_only" />
            ...
            <!-- Playlist -->
            <div class="flex items-center space-x-2 h-10 px-3 border rounded-md bg-card">
              <Checkbox id="playlist" v-model:checked="form.playlist" />
        ...
        <div class="space-y-4">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
            <div class="flex items-center space-x-2 h-10 px-3 border rounded-md bg-card">
              <Checkbox id="embed_chapters" v-model:checked="form.embed_chapters" />
            ...
            <div class="flex items-center space-x-2 h-10 px-3 border rounded-md bg-card">
              <Checkbox id="embed_metadata" v-model:checked="form.embed_metadata" />
            ...
            <div class="flex items-center space-x-2 h-10 px-3 border rounded-md bg-card">
              <Checkbox id="embed_thumbnail" v-model:checked="form.embed_thumbnail" />
            ...
            <div class="flex items-center space-x-2 h-10 px-3 border rounded-md bg-card">
              <Checkbox id="embed_subtitles" v-model:checked="form.embed_subtitles" />
            ...
            <div class="flex items-center space-x-2 h-10 px-3 border rounded-md bg-card">
              <Checkbox id="split_chapters" v-model:checked="form.split_chapters" />
        ...
        <div class="space-y-4">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div class="flex items-center space-x-2 h-10 px-3 border rounded-md bg-card">
              <Checkbox id="write_info_json" v-model:checked="form.write_info_json" />
              <Label for="write_info_json" class="cursor-pointer font-normal">📄 Info JSON</Label>
            </div>
            <div class="flex items-center space-x-2 h-10 px-3 border rounded-md bg-card">
              <Checkbox id="write_description" v-model:checked="form.write_description" />
              <Label for="write_description" class="cursor-pointer font-normal">📋 Description</Label>
            </div>
            <div class="flex items-center space-x-2 h-10 px-3 border rounded-md bg-card">
              <Checkbox id="write_thumbnail" v-model:checked="form.write_thumbnail" />
        ...
        <div class="space-y-4">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div class="flex items-center space-x-2 h-10 px-3 border-2 border-purple-500 rounded-md bg-purple-50 dark:bg-purple-950">
              <Checkbox id="sponsorblock" v-model:checked="form.sponsorblock" class="data-[state=checked]:bg-purple-600" />
            ...
            <div class="flex items-center space-x-2 h-10 px-3 border rounded-md bg-card">
              <Checkbox id="audio_multistreams" v-model:checked="form.audio_multistreams" />
        ...
        <Button
          ...
          size="lg"
        >
          {{ loading ? '⏳ Wird gestartet...' : '🚀 Download starten' }}
        </Button>
      </CardContent>
    </Card>
      ...
      <CardContent>
        ...
        <div v-else class="space-y-3">
          <Card
            ...
            :class="{
              'border-l-4 border-l-green-500': dl.status === 'completed',
              'border-l-4 border-l-blue-500': dl.status === 'downloading',
              'border-l-4 border-l-yellow-500': dl.status === 'pending',
              'border-l-4 border-l-red-500': dl.status === 'failed'
            }"
          >
              ...
              <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div class="flex-1 min-w-0 space-y-2">
                  <div class="flex items-center gap-2 flex-wrap">
                    <h3 class="font-semibold truncate">{{ dl.title || dl.url }}</h3>
                    <Badge
                  ...
                  <p class="text-sm text-muted-foreground truncate">{{ dl.url }}</p>
                  <p class="text-xs text-muted-foreground">{{ formatDate(dl.created_at) }}</p>
                  <div v-if="dl.status === 'downloading'" class="space-y-1">
                    <Progress :model-value="dl.progress" class="h-2" />
                    <p class="text-xs text-muted-foreground">{{ dl.progress }}%</p>
                  </div>
                </div>
                <div v-if="dl.status === 'completed' || dl.status === 'failed'" class="flex gap-2 flex-wrap shrink-0">
                  <Button
                  ...
                  <Button
                    ...
                    @click="downloadFolder(dl.job_id)"
                    variant="default"
                    size="sm"
                    class="bg-purple-600 hover:bg-purple-700"
                  >
                    📦 {{ dl.is_playlist ? 'Playlist' : 'Ordner' }}
                  </Button>
                  <AlertDialog>
                    ...
                    <AlertDialogContent>
                      <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                          Diese Aktion kann nicht rückgängig gemacht werden. Alle Dateien werden permanent gelöscht.
                        </AlertDialogDescription>
                      </AlertDialogHeader>
                      <AlertDialogFooter>
                        <AlertDialogCancel>Abbrechen</AlertDialogCancel>
                        <AlertDialogAction @click="deleteDownload(dl.job_id)" class="bg-destructive text-destructive-foreground hover:bg-destructive/90">
                          Löschen
                        </AlertDialogAction>
                      </AlertDialogFooter>
                    </AlertDialogContent>
